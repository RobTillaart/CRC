...
inline uint8_t reverse8bits(uint8_t in)
{
  uint8_t x = in;
  x = (((x & 0xAA) >> 1) | ((x & 0x55) << 1));
  x = (((x & 0xCC) >> 2) | ((x & 0x33) << 2));
...
inline uint16_t reverse16bits(uint16_t in)
{
  uint16_t x = in;
...
inline uint16_t reverse12bits(uint16_t in)
{
  return reverse16bits(in) >> 4;
}

inline uint32_t reverse32bits(uint32_t in)
{
  uint32_t x = in;
...
inline uint64_t reverse64bits(uint64_t in)
{
  uint64_t x = in;
